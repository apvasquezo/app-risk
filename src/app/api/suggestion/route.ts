export async function POST(request: Request) {
  ...
  **Instrucciones:**
  ...
  Asegúrate de que cada sugerencia sea concisa, específica y directamente aplicable al riesgo descrito.`
  
      const response = await fetch('https://api.openai.com/v1/chat/completions', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${process.env.API_KEY_OPENAI}`, // Sin NEXT_PUBLIC_
        },
        body: JSON.stringify({
          model: 'gpt-3.5-turbo',
          messages: [{ 
